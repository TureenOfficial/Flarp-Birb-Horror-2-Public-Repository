...
{
    public RuntimeUtils utilLib;
    public int chapterNumber = 1;
    public string niceTime = "02:30";

    public void CheckTime(string gameTitle)
    {
        chapterNumber = utilLib.chapterNumber;
        niceTime = utilLib.niceTime;

        string bestTime = PlayerPrefs.GetString($"{gameTitle}chapter{chapterNumber}CompletionTime", "99:99");
        ...
        if (niceTimeInSeconds < bestTimeInSeconds)
        {
            ...
            PlayerPrefs.SetString($"{gameTitle}chapter{chapterNumber}CompletionTime", niceTime);
